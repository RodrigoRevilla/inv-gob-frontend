...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-crema flex flex-col">
      ...
      <header class="bg-guinda-700 border-b-4 border-dorado sticky top-0 z-40 shadow-md">
        <div class="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div class="border-l border-white/20 pl-3">
              <div class="text-white font-semibold text-sm truncate max-w-[200px] sm:max-w-sm">
                {{ sesion()?.nombre_sesion ?? 'Cargando...' }}
              </div>
              <div class="text-dorado-light text-xs">Detalle de sesión</div>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2">
            @if (sesion() && !cargando()) {
              <button (click)="exportarFaltantes()"
                      ...
                      title="Exportar faltantes a Excel"
              ...
              <button (click)="exportarEscaneos()"
                      ...
                      title="Exportar escaneos a Excel"
                      class="text-white/70 hover:text-white text-xs border border-white/20
                             hover:border-green-400 hover:text-green-300 px-3 py-1.5
                             rounded-lg transition-all duration-150 flex items-center gap-1.5
                             disabled:opacity-30 disabled:cursor-not-allowed">
