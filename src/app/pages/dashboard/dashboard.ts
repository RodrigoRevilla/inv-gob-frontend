      ...
      <div class="bg-guinda-700 border-b-4 border-dorado sticky top-0 z-50 shadow-md">
        <div class="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
          <div class="flex items-center gap-4">
            <img src="/oaxaca.png" alt="Escudo Oaxaca" class="h-9 w-9 object-contain"/>
          ...
          <div class="flex items-center gap-3">
            <div class="hidden sm:flex items-center gap-2">
              <div class="w-2 h-2 rounded-full" [class]="rolDot(usuario()?.rol)"></div>
            ...
            @if (esAdminOAuditor()) {
              <button (click)="router.navigate(['/audit-log'])"
                      class="text-white/70 hover:text-white text-sm border border-white/20
                             hover:border-white/40 px-3 py-1.5 rounded-lg transition-all duration-150">
                Actividad
              </button>
...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  crearSesion() {
    ...
    this.api.crearSesion(this.nuevaSesion).subscribe({
      next: res => {
        ...
        if (res.ok && res.data) { this.cerrarModal(); this.router.navigate(['/escaneo', res.data.id]); }
  ...
  irSeccion(s: ResumenSesion, seccion: string, event: Event) {
    ...
    this.router.navigate(['/sesion', s.sesion_id], { fragment: seccion });
  }
  ...
  irSesion(s: ResumenSesion) {
    this.router.navigate([s.estado === 'abierta' ? '/escaneo' : '/sesion', s.sesion_id]);
  }
